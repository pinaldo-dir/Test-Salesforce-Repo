...
//import OWCViolationStyle from '@salesforce/resourceUrl/OWCViolationStyle'
...
import generateInterviewPDF from '@salesforce/apex/OnlineWageClaimContainerController.generateInterviewPDF';
import updateCaseRec from '@salesforce/apex/OnlineWageClaimContainerController.updateCaseRec';
import createOWCJSONResponse from '@salesforce/apex/OWCSaveDraftController.createOWCJSONResponse';
import getOWCGarmentJSONResponse from '@salesforce/apex/OWCGarmentInterviewDraftController.getOWCGarmentJSONResponse';
import updateCaseRecWithJSON from '@salesforce/apex/OWCGarmentInterviewDraftController.updateCaseRecWithJSON';
...
export default class OnlineWageClaimGarmentContainerCmp extends LightningElement {
    @track currentStep = '1';
    ...
    console.log('currentCommunityUrl ::: '+this.recordId)
    if(this.recordId != null){
        getOWCGarmentJSONResponse({
            recordId : this.recordId,
            draftRecordId : this.draftRecordId
        })
           .then(result => {
               if(result){
                   console.log('result ::: ', JSON.stringify(result.jsonObj))
                   this.currentStep = "1"; 
                   console.log('current step ::: ', this.currentStep)
                   //this.currentStep = result.Section_Id__c;    
                    const finalResult = JSON.parse(result.jsonObj.replace(/&quot;/g,'"').replaceAll(/&copy;/g,'©').replaceAll(/&amp;/g,'&').replaceAll(/&lt;/g,'<').replaceAll(/&gt;/g,'>').replaceAll(/&#39;/g,"'"));
                    ...
                    if(this.isFirstCmpRender === true){
                        ...
                        this.template.querySelector('c-owc-Preliminary-Garment-Section-Cmp').owcClaimantEmployeeInfoForChild(this.claimantEmployeeValues)
    ...
    handleNext(event){
        ...
        switch ( this.currentStep ) {  
            ...
            //     // this.template.querySelector('c-owc-Preliminary-Garment-Section-Cmp').owcClaimantEmployeeDataForParent();
            ...
            case "2":
                ...
                this.template.querySelector('c-industry-info-garment-cmp ').owcInformationDataForParent()
            ...
            break;
            case "3":
                this.template.querySelector('c-owc-Emp-Business-Garment-Type-Container-Cmp').owcEmpDataForContainerParent(false);
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            else{
            ...
            break;
            case "4":
                this.template.querySelector('c-owc-Other-Work-Location-Garment-Section-Cmp').otherWorkLocationDetailsFromParent(false);
            ...
            break;
            case "5":
                // this.template.querySelector('c-owc-Other-Work-Location-Garment-Section-Cmp').otherWorkLocationDetailsFromParent();
                this.template.querySelector('c-owc-Employment-Status-And-Final-Wages-Garment-Cmp').owcEmpStatusAndWagesDataForParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            else{
            ...
            case "6":
                // this.template.querySelector('c-owc-Employment-Status-And-Final-Wages-Garment-Cmp').owcEmpStatusAndWagesDataForParent();
                this.template.querySelector('c-owc-Payment-Of-Wages-Garment-Cmp').owcPaymentOfWagesFromParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            else{
            ...
            case "7":
                ...
                // this.template.querySelector('c-owc-Payment-Of-Wages-Garment-Cmp').owcPaymentOfWagesFromParent();
                this.template.querySelector('c-owc-Workweek-And-Workday-Garment-Cmp').owcWorkWeekAndWorkDayForParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            else{
            ...
            break;
            case "8":
                this.template.querySelector('c-owc-Hours-You-Typically-Worked-Garment-Cmp').owcHoursYouWorkedForParent();
                // this.template.querySelector('c-owc-Hours-You-Typically-Worked-Garment-Cmp').owcHoursYouWorkedForParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            ...
            break;
            case "9":
                this.template.querySelector('c-owc-Wage-Deficiencies-Garment').owcwageDeficienciesFromParent();
                // this.template.querySelector('c-owc-Workweek-And-Workday-Garment-Cmp').owcWorkWeekAndWorkDayForParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            ...
            break;
            case "10":
                this.template.querySelector('c-owc-Label-Section-Container-Garment-Cmp').labelSectionFromParent();
            // this.template.querySelector('c-owc-Wage-Deficiencies-Garment').owcwageDeficienciesFromParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false
            }
            ...
            break;
            case "12":
                this.template.querySelector('c-owc-Online-Demographic-Garment-Cmp').owcOnlineDemographicForParent();
            if(this.isNotChangedCurrentStep === true){
                this.isNotChangedCurrentStep = false;
            }
            else{
    ...
    handlePrev(event){
        ...
        switch ( this.currentStep ) {  
            ...
            case "11":
                this.handlerCmpRenderFun("12", false, false, false, false, false, false, false, false, false, false, false, true)
                // this.template.querySelector('c-owc-Certification-And-Authorization-Garment-Cmp').certAndAuthInfoFromParent();
            ...
            case "4":
                this.handlerCmpRenderFun("3", false, false, true, false, false, false, false, false, false, false, false, false)
                this.template.querySelector('c-owc-Other-Work-Location-Garment-Section-Cmp').otherWorkLocationDetailsFromParent(true);
                break;
            case "3":
                this.handlerCmpRenderFun("2", false, true, false, false, false, false, false, false, false, false, false, false)
                this.template.querySelector('c-owc-Emp-Business-Garment-Type-Container-Cmp').owcEmpDataForContainerParent(true);
                break;
            case "2":
                this.handlerCmpRenderFun("1", true, false, false, false, false, false, false, false, false, false, false, false)
    ...
    @track isWageDefGarmentClaim = true;
    ...
    handleIndustryInfoSectionData(event){
        ...
        // if(this.industryInfoDataValues.isLabelsInfo === true  && this.industryInfoDataValues.isGarmentDutyFollow != 'No'){
    ...
    renderedCallback(){   
        ...
        if(this.isThirdCmpRender === true && this.employeesDetails.length > 0){
            ...
            this.template.querySelector('c-owc-Emp-Business-Garment-Type-Container-Cmp').owcEmployeesDataForChildCmp(this.employeesDetails, this.renderEmpSection );
        ...
        //     this.template.querySelector('c-owc-Emp-Business-Garment-Type-Container-Cmp').owcClaimentInfo(this.claimantEmployeeValues.WageClaimSubmit, this.industryInfoDataValues.isLabelsInfo)
        //     this.isThirdCmpRender = false
        // }
        if(this.isSecondCmpRender === true && this.industryInfoDataValues != undefined){
            this.template.querySelector('c-industry-info-garment-cmp ').owcIndustryInfoForChild(this.industryInfoDataValues)
            console.log('isSecondCmpRender:', JSON.stringify(this.industryInfoDataValues));
            //this.template.querySelector('c-industry-info-garment-cmp ').owcClaimentInfo(this.claimantEmployeeValues.WageClaimSubmit)
                        
        }
        if(Boolean(this.demoGraphicInfoData) && this.isTwelveCmpRender === true ){
            this.template.querySelector('c-owc-Online-Demographic-Garment-Cmp').owcOnlineDemographicForChild(this.demoGraphicInfoData)
            this.isTwelveCmpRender = false
        }
        if(this.isFourthCmpRender === true && this.otherLocWorkInfo != null){
            this.template.querySelector('c-owc-Other-Work-Location-Garment-Section-Cmp').handleOtherWorkLocationChild(this.otherLocWorkInfo)
            this.isFourthCmpRender = false
        }
        if(this.isFifthCmpRender === true && this.employeeStatusAndFinalClaimDetails != null){
            this.template.querySelector('c-owc-Employment-Status-And-Final-Wages-Garment-Cmp').owcEmpStatusAndWagesDataForChild(this.employeeStatusAndFinalClaimDetails);
            this.isFifthCmpRender = false
        }
        if(this.isSixthCmpRender === true && this.paymentOfWagesDetails != null){
            this.template.querySelector('c-owc-Payment-Of-Wages-Garment-Cmp').owcPaymentOfWagesForChild(this.paymentOfWagesDetails);
            this.isSixthCmpRender = false
        }
        // if(this.isSixthCmpRender === true  && this.claimantEmployeeValues.WageClaimSubmit != null){
        //     // this.template.querySelector('c-owc-Payment-Of-Wages-Garment-Cmp').owcClaimentInfo(this.claimantEmployeeValues.WageClaimSubmit, this.industryInfoDataValues.isLabelsInfo)
            
        // }
        if(this.isEighthCmpRender === true && this.hoursYouTypicallyWorkedDetails != null){
            this.template.querySelector('c-owc-Hours-You-Typically-Worked-Garment-Cmp').owcHoursYouWorkedForChild(this.hoursYouTypicallyWorkedDetails);
        ...
        if(this.isSeventhCmpRender === true && this.workWeekAndWorkDaysDetails != null){
            ...
            this.template.querySelector('c-owc-Workweek-And-Workday-Garment-Cmp').owcEmpStatusAndWagesDataForChild(this.workWeekAndWorkDaysDetails);
        ...
        if(this.isTenthCmpRender === true && this.labelDetails != null){
            this.template.querySelector('c-owc-Label-Section-Container-Garment-Cmp').labelSectionForChild(this.labelDetails);
        ...
        if(this.isNinethCmpRender === true && (this.paymentOfWagesDetails !== null || this.paymentOfWagesDetails !== undefined)){
            this.template.querySelector('c-owc-Wage-Deficiencies-Garment').handlepaymentofwagesdetails(this.paymentOfWagesDetails, this.isFormPreviewMode);
        }
        if(this.isNinethCmpRender === true && (this.employeeStatusAndFinalClaimDetails !== undefined)){
            this.template.querySelector('c-owc-Wage-Deficiencies-Garment').handleEmpStatusValue(this.employeeStatusAndFinalClaimDetails.QuitDateTemplate, this.employeeStatusAndFinalClaimDetails.DischargeDateTemplate);
        }
        if(this.isNinethCmpRender === true && (this.wageDeficiencyDetails !== undefined || this.wageDeficiencyDetails !== null)){
            this.template.querySelector('c-owc-Wage-Deficiencies-Garment').owcWageDeficienciesForChild(this.wageDeficiencyDetails, this.renderWageDefSection, this.isFormPreviewMode);
            this.isNinethCmpRender = false
        }
        if(this.isFormPreviewMode === true){
            this.template.querySelector('c-owc-Garment-Form-Preview-Cmp').getFormDetails(this.formDetailObj, this.isFormPreviewMode, this.isClaimantGarment, this.isClaimantAdvocate, this.renderWageDefSection);
        }
        ...
        //     this.template.querySelector('c-owc-Certification-And-Authorization-Garment-Cmp').certAndAuthInfoForChild(this.certAndAuthDetails);
    ...
    handleOnSubmitOperation(){
        ...
        // this.isClaimantAdvocate === false ? this.template.querySelector('c-owc-Certification-And-Authorization-Garment-Cmp').certAndAuthInfoFromParent() : ''
        if(this.isNotChangedCurrentStep === true){
            this.isNotChangedCurrentStep = false;
            return;
        }
        ...
        onlineClaimSubmit({
               ...
               .then(result => {
                   if(result && result != 'TEST'){
                        this.handleGeneratePDF(result);
                        // this.isShowSpinner = false;
                        // this.showToast('success', 'Your Claim has been successfully submitted.','success')
    ...
    handleGeneratePDF(caseRecordId){
    ...
    hanleInterviewPDFGeneration(event){
        event.preventDefault();
        this.isShowSpinner = true;
        updateCaseRec({
            onlineClaimDetails : JSON.stringify(this.handleOnlineFormDetails()),
        ...
        .then(result => {
            if(Boolean(result) && Boolean(result.isSuccess)){
                this.callPdfController();
            }
            else{
                this.isShowSpinner = false;
            }
        ...
        .catch(error => {
            if(Boolean(error) && Boolean(error.body) && Boolean(error.body.message)){
                this.isShowSpinner = false;
    ...
    async handleConfirm(){
        const result = await LightningConfirm.open({
            message: "Canceling will delete all information that has been entered, including any saved drafts.  Are you sure you want to cancel this interview?",
            theme: "warning",
            label:"Warning!"
        });
        if(result){
    ...
    callPdfController(){
        ...
        .then(result => {
            if(Boolean(result)){
                this.isShowSpinner = false;
                this.showToast('Success', 'Your interview PDF has successfully created.', 'Success');
                this.handleCancel();
            }
            else{
                this.isShowSpinner = false;
