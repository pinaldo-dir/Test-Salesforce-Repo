...
export default class OwcNotAllowedRestBreakChild extends LightningElement {
    // Attributes
    @api sectionid;
    @api additionalName;
    ...
    async handleHourlyRateOption(){
        try{
            await this.getAllVilationTypeVariablesForOneHourRateRec('61');
        }catch (error){
        ...
        //     try{
        //         await this.getAllVilationTypeVariablesForSalaryRateRec('63');
        //     }catch (error){
        //         console.log('error', error);
        //     }
        // }
        ...
        .then(result => {
            if(result){
                for(var i=0; i<result.length; i++){
                    ...
                    result[i].name === 'VTV0078' || result[i].name === 'VTV1002' ? result[i].isHide = true : result[i].isHide = false
                    result[i].name === 'VTV0075' ? result[i].helpText = true : result[i].helpText = false;
                ...
                // Check the datatype of the input fields
                 ...
                 this.iwcinfoobj.iwcOrderNumber !== undefined ? this.violationTypeVariablesForOneHourlyRate.forEach( element => element.name === 'VTV0078' ? element.value = this.iwcinfoobj.iwcOrderNumber : '' ) : ''
                 this.iwcinfoobj.iwcOrderNumber !== undefined ? this.violationTypeVariablesForOneHourlyRate.forEach( element => element.name === 'VTV1002' ? element.value = '12' : '' ) : ''
            }
            console.log('violationtype variable :::',this.violationTypeVariablesForOneHourlyRate)
            console.log('isRequired ::::',this.isRequiredField)
        })
        .catch(error => {
            console.log('Error: ', error);
        })
        ...
        .then(result => {
            if(result){
                for(var i=0; i<result.length; i++){
                 ...
                 this.iwcinfoobj.iwcOrderNumber !== undefined ? this.violationTypeVariablesForOneSalaryRate.forEach( element => element.name === 'VTV1677' || element.name === 'VTV1676' ? element.value = this.iwcinfoobj.iwcOrderNumber : '' ) : ''
            }
        })
        .catch(error => {
            console.log('Error: ', error);
    ...
    handleOverTimeParentData(){
        ...
        if(this.isOneHourlyRatePOW === true){
            ...
            const   claimStartDate = this.template.querySelector('[data-id="VTV0073"]')
            const   claimEndDate = this.template.querySelector('[data-id="VTV0074"]')
         //   const   hourlyratepay = this.template.querySelector('[data-id="VTV0075"]')
            const   numberofwrkdays = this.template.querySelector('[data-id="VTV0076"]')
            const   howmuchpay  = this.template.querySelector('[data-id="VTV0077"]')
    ...
    handleOverTimeParentInfo(){
        const selectEvent = new CustomEvent('overtimecustominfoevent', {
            detail:{
                restBreakInfoObj : this.restBreakInfoObj() ,
