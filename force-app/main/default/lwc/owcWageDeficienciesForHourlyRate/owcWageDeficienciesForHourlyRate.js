...
let fileSize=0;
export default class OwcWageDeficienciesForHourlyRate extends LightningElement {
    @track fileNames = '';
    @track filesUploaded = [];
    ...
    isHoursWorkedMissing = false;
    isTotalAmountMissing = false;
    isHourlyRateMissing = false;
    isPeriodFrequencyMissing = false;
    isRateofPayMissing = false;
    isTotalPieceRateAmountMissing = false; 
    ...
    handleWageDeficienciesParent(){
        ...
        if(this.isOneHourlyRatePOW === true){
             ...
             const hoursWorkedId = `[data-id="${this.violationTypeVariablesForOneHourlyRate[2].name}"]`;
             const hourlyPayId = `[data-id="${this.violationTypeVariablesForOneHourlyRate[3].name}"]`;
             const totalAmtId  = `[data-id="${this.violationTypeVariablesForOneHourlyRate[4].name}"]`;
             ...
             const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
             const hourlyRateOfPayDuringPeriod = this.template.querySelector(String(hourlyPayId));
             const totalAmountPaid = this.template.querySelector(String(totalAmtId));
             ...
             this.isHoursWorkedMissing = this.handleRequiredField(hoursWorkedDuringPeriod);
             this.isHourlyRateMissing = this.handleRequiredField(hourlyRateOfPayDuringPeriod);
             this.isTotalAmountMissing = this.handleRequiredField(totalAmountPaid);
             
             this.isDateAcceptable === true ? endingDateId.focus() : ''
             this.isHourlyRateMissing === true ? hourlyPayId.focus() : ''
             this.isHoursWorkedMissing === true ? hoursWorkedId.focus() : ''
             this.isTotalAmountMissing === true ? totalAmtId.focus() : ''

        }

        if(this.isDifferentHourPOW === true){
            const beggingDateId = `[data-id="${this.violationTypeVariablesForDifferentHourlyRate[1].name}"]`;
            const endingDateId = `[data-id="${this.violationTypeVariablesForDifferentHourlyRate[2].name}"]`;
            const hoursWorkedId = `[data-id="${this.violationTypeVariablesForDifferentHourlyRate[3].name}"]`;
            const totalAmtId = `[data-id="${this.violationTypeVariablesForDifferentHourlyRate[4].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
        ...
        if(this.isSalaryRatePOW === true){
            const beggingDateId = `[data-id="${this.violationTypeVariablesForOneSalaryRate[0].name}"]`;
            const endingDateId = `[data-id="${this.violationTypeVariablesForOneSalaryRate[1].name}"]`;
            const periodFrequencyId = `[data-id="${this.violationTypeVariablesForOneSalaryRate[2].name}"]`;
            const hourlyPayId = `[data-id="${this.violationTypeVariablesForOneSalaryRate[3].name}"]`;
            const hoursWorkedId = `[data-id="${this.violationTypeVariablesForOneSalaryRate[4].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForOneSalaryRate[5].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
            const hourlyRateOfPayDuringPeriod = this.template.querySelector(String(hourlyPayId));
            const payPeriodFrequency = this.template.querySelector(String(periodFrequencyId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
            this.isDateAcceptable = this.handlePOWDateValidity(hourlyRateBeginningDate, hourlyRateEndingDate, hourlyRateBeginningDate.value, hourlyRateEndingDate.value);
            this.isPeriodFrequencyMissing = this.handleRequiredField(payPeriodFrequency);
            this.isHoursWorkedMissing = this.handleRequiredField(hoursWorkedDuringPeriod);
            this.isHourlyRateMissing = this.handleRequiredField(hourlyRateOfPayDuringPeriod);
            this.isTotalAmountMissing = this.handleRequiredField(totalAmountPaid);
            ...
            this.isDateAcceptable === true ? endingDateId.focus() : ''
            this.isPeriodFrequencyMissing === true ? periodFrequencyId.focus() : ''
            this.isHourlyRateMissing === true ? hourlyPayId.focus() : ''
            this.isHoursWorkedMissing === true ? hoursWorkedId.focus() : ''
            this.isTotalAmountMissing === true ? totalAmtId.focus() : ''
        ...
        if(this.isNotCommissionAgreementUpload === true){
            ...
            const endingDateId = `[data-id="${this.violationTypeVariablesForCommissionRate[2].name}"]`;
            const hoursWorkedId = `[data-id="${this.violationTypeVariablesForCommissionRate[3].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForCommissionRate[4].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
        ...
        if(this.isCommissionAgreementUpload === true){
            ...
            const hoursWorkedId = `[data-id="${this.violationTypeVariablesForCommissionRateUpload[2].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForCommissionRateUpload[3].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
        ...
        if(this.isOnePieceRatePOW === true){
            const beggingDateId = `[data-id="${this.violationTypeVariablesForOnePieceRate[0].name}"]`;
            const endingDateId = `[data-id="${this.violationTypeVariablesForOnePieceRate[1].name}"]`;
            const piecesProducedId = `[data-id="${this.violationTypeVariablesForOnePieceRate[2].name}"]`;
            const typesOfPieceId = `[data-id="${this.violationTypeVariablesForOnePieceRate[3].name}"]`;
            const rateofPayId = `[data-id="${this.violationTypeVariablesForOnePieceRate[4].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForOnePieceRate[5].name}"]`;
            const totalPieceAmtId = `[data-id="${this.violationTypeVariablesForOnePieceRate[6].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const piecesProduceddDuringPeriod = this.template.querySelector(String(piecesProducedId));
            const typesOfPieces = this.template.querySelector(String(typesOfPieceId));
            const rateofPayPerPiece = this.template.querySelector(String(rateofPayId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
            const totalPieceRateAmoutPaid = this.template.querySelector(String(totalPieceAmtId));
            this.isDateAcceptable = this.handlePOWDateValidity(hourlyRateBeginningDate, hourlyRateEndingDate, hourlyRateBeginningDate.value, hourlyRateEndingDate.value);
            this.isHoursWorkedMissing = this.handleRequiredField(piecesProduceddDuringPeriod);
            this.isHourlyRateMissing = this.handleRequiredField(typesOfPieces);
            this.isRateofPayMissing = this.handleRequiredField(rateofPayPerPiece);
            this.isTotalAmountMissing = this.handleRequiredField(totalAmountPaid);
            this.isTotalPieceRateAmountMissing = this.handleRequiredField(totalPieceRateAmoutPaid);
            
            this.isDateAcceptable === true ? endingDateId.focus() : ''
            this.isHourlyRateMissing === true ? typesOfPieceId.focus() : ''
            this.isHoursWorkedMissing === true ? piecesProducedId.focus() : ''
            this.isRateofPayMissing === true ? rateofPayId.focus() : ''
            this.isTotalAmountMissing === true ? totalAmtId.focus() : ''
            this.isTotalPieceRateAmountMissing === true ? totalPieceAmtId.focus() : ''

        }

        if(this.isDifferentPieceRatePOW === true){
            const beggingDateId = `[data-id="${this.violationTypeVariablesForDifferentPieceRate[0].name}"]`;
            const endingDateId = `[data-id="${this.violationTypeVariablesForDifferentPieceRate[1].name}"]`;
            const pieceRatesTypesId = `[data-id="${this.violationTypeVariablesForDifferentPieceRate[2].name}"]`;
            const pieceworkPaidId = `[data-id="${this.violationTypeVariablesForDifferentPieceRate[3].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForDifferentPieceRate[4].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(pieceRatesTypesId));
            const hourlyRateOfPayDuringPeriod = this.template.querySelector(String(pieceworkPaidId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
            this.isDateAcceptable = this.handlePOWDateValidity(hourlyRateBeginningDate, hourlyRateEndingDate, hourlyRateBeginningDate.value, hourlyRateEndingDate.value);
            this.isHoursWorkedMissing = this.handleRequiredField(hoursWorkedDuringPeriod);
            this.isHourlyRateMissing = this.handleRequiredField(hourlyRateOfPayDuringPeriod);
            ...
            this.isDateAcceptable === true ? endingDateId.focus() : ''
            this.isHourlyRateMissing === true ? pieceworkPaidId.focus() : ''
            this.isHoursWorkedMissing === true ? pieceRatesTypesId.focus() : ''
            this.isTotalAmountMissing === true ? totalAmtId.focus() : ''

        }

        if(this.isPieceRateUnknown === true){
            const beggingDateId = `[data-id="${this.violationTypeVariablesForUnknownPieceRate[0].name}"]`;
            const endingDateId = `[data-id="${this.violationTypeVariablesForUnknownPieceRate[1].name}"]`;
            const hoursWorkedId = `[data-id="${this.violationTypeVariablesForUnknownPieceRate[2].name}"]`;
            const totalAmtId  = `[data-id="${this.violationTypeVariablesForUnknownPieceRate[3].name}"]`;
            const hourlyRateBeginningDate = this.template.querySelector(String(beggingDateId));
            const hourlyRateEndingDate = this.template.querySelector(String(endingDateId));
            const hoursWorkedDuringPeriod = this.template.querySelector(String(hoursWorkedId));
            const totalAmountPaid = this.template.querySelector(String(totalAmtId));
        ...
        if(this.isUnknownDateAcceptable === true || this.isUploadCommissionDateAcceptable === true || this.isCommissionDateAcceptable === true 
        || this.isTotalAmountMissing === true || this.isHoursWorkedMissing === true || this.isHourlyRateMissing === true || this.isPeriodFrequencyMissing === true 
        || this.isDateAcceptable === true || this.isSalaryDateAcceptable === true || this.isDifferentSalaryDateAcceptable === true || this.isRateofPayMissing === true
        || this.isDifferentPieceDateAcceptable === true || this.isOnePieceDateAcceptable === true || this.isTotalPieceRateAmountMissing === true){
           this.workNotPaidValidityChecker();
        }
