...
export default class OwcWaitingTimeClaim extends LightningElement {
    @api powdetails;
        ...
        .then(result => {
            if(result){
                for(var i=0; i<result.length; i++){
                    ...
                    result[i].name === 'VTV1015' || result[i].name === 'VTV1018' ? result[i].isRequiredField = true : result[i].isRequiredField = false;
                    result[i].value = '';
                }
    ...
    @api isNumberOfDaysMissing = false;
    @api isTotalAmountPaidMissing = false;
    ...
    handleWaitingTimeParent(){
        ...
        if(this.dischargeddate !== null){
            let NumberOfDays = this.template.querySelector('[data-id="VTV1015"]');
            let TotalAmountPaid = this.template.querySelector('[data-id="VTV1018"]');

            this.isNumberOfDaysMissing = this.handleRequiredField(NumberOfDays)
            this.isTotalAmountPaidMissing = this.handleRequiredField(TotalAmountPaid)

            this.isNumberOfDaysMissing === true? NumberOfDays.focus() : ''
            this.isTotalAmountPaidMissing === true ? TotalAmountPaid.focus() : ''

            console.log('console missing days', this.isNumberOfDaysMissing)
        }

        if(this.isNameValid === true || this.isNumberOfDaysMissing === true ||this.isTotalAmountPaidMissing === true){
            this.handleEmployerValidityEvent();
        }
    }
    ...
    handleChange(event){
        ...
        if(this.violationTypeVariablesForWaitingTimeOne !== undefined){
            ...
            if(this.dischargeddate !== null){
                let NumberOfDays = this.template.querySelector('[data-id="VTV1015"]');
                let TotalAmountPaid = this.template.querySelector('[data-id="1018"]');
                this.isNumberOfDaysMissing = this.handleRequiredField(NumberOfDays)
                this.isTotalAmountPaidMissing = this.handleRequiredField(TotalAmountPaid)
            }
        }
